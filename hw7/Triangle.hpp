                          ...
                          const Vector3f& dir, float& tnear, float& u, float& v)
{
    ...
    // please comment this line before implementation
    auto edge1 = v1-v0;
    auto edge2 = v2-v0;
    auto s = orig-v0;
    ...
    float co = 1/(dotProduct(s1,edge1));
    tnear = co*dotProduct(s2,edge2);
    u = co*dotProduct(s1,s);
    v = co*dotProduct(s2,dir);
...
inline Intersection Triangle::getIntersection(Ray ray)
{
    ...
    float s1e1_inv = 1 / dotProduct(s1, e1); // inverse of s1 dot product e1
    t = dotProduct(s2, e2) * s1e1_inv;
    u = dotProduct(s1, s) * s1e1_inv;
    v = dotProduct(s2, dir) * s1e1_inv;
    if(t>=0&&u>=0&&v>=0&&(u+v)<=1){
    inter.obj = dynamic_cast<Object*>(this);
    inter.m = this->m;
    inter.happened = t > 0 && u >= 0 && v >= 0 && u + v <= 1;
